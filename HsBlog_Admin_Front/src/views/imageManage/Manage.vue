...
import {randomUUID} from "@/utils/imageutils/uuid";
import {copy} from "@/utils/imageutils/copy";
import {taskQueue} from "@/utils/imageutils/task-queue";
export default {
  name: "Manage",
  components: {SvgIcon},
  data() {
  ...
  created() {
    ...
    if (token && userInfo) {
      ...
      this.errorMsg('请先配置Token')
      this.$router.push('/imageManage/setting')
    }
  },
  methods: {
    //获取用户仓库
