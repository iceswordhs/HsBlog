        ...
        <div class="ui grid m-margin-lr">
          ...
          <div class="row m-padded-tb-small">
            <div class="ui horizontal link list m-center">
              ...
              <div class="item m-common-black">
                <i class="small clock icon"></i><span>阅读时长≈{{ parseInt(blog.words/500) }}分</span>
              </div>
...
<!--          </div>-->
          ...
          <div class="row m-padded-tb-no">
            <div class="column m-padding-left-no">
              <router-link :to="`/tag/${tag.name}`" class="ui tag label m-text-500 m-margin-small" :class="tag.color" v-for="(tag,index) in blog.articleTags" :key="index">{{ tag.tagName }}</router-link>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="ui attached positive message">
      <ul class="list">
        ...
        <li>最后修改：{{ blog.updateTime|dateFormat}}</li>
    ...
    getBlog (id = this.blogId) {
      ...
      getBlogById(token, id).then(res => {
        if (res.data.code === 200) {
          ...
          // document.title = this.blog.articleTitle + this.siteInfo.webTitleSuffix
